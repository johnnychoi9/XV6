...
scheduler(void)
{
  ...
  for(;;){
    ...
    acquire(&ptable.lock);
    int allofTheTickets = 0;
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
      if(p->state != RUNNABLE)
        continue;
      allofTheTickets += p->totalTickets;
    }
    if(allofTheTickets > 0){
    int newProcess = (next_random() % allofTheTickets) + 1;
    for(p = ptable.proc; p < &ptable.proc[NPROC]; p++){
      if(p->state != RUNNABLE)
        continue;
      ...
      break;
        ...
        newProcess = newProcess - p->totalTickets;
      }
    }
  }
    release(&ptable.lock);
...
int getprocessesinfo(struct processes_info* p)
{
  ...
  for(t = ptable.proc; t < &ptable.proc[NPROC]; t++){
    if(t->state != UNUSED){
      p->pids[p->num_processes] = t->pid;
      p->ticks[p->num_processes] = t->ticks;
      p->tickets[p->num_processes] = t->totalTickets;
      p->num_processes++;
    }
  }
  release(&ptable.lock);
